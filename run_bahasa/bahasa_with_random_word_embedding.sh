...
#dataroot=data/bahasa_both/$domain
#dataset=bahasa_both_random_$domain
dataroot=data/bahasa_both/$domain
dataset=bahasa_both_random_$domain

word_embedding_size=768 #400 #1024
...
python scripts/slot_tagging_and_intent_detection.py --task_st $task_slot_filling --task_sc $task_intent_detection --dataset $dataset \
      --dataroot $dataroot --bidirectional --lr $learning_rate --dropout $dropout_rate --batchSize $batch_size --optim $optimizer \
      --max_norm $max_norm_of_gradient_clip --experiment $experiment_output_path --deviceId $device --max_epoch $max_epoch \
      --emb_size $word_embedding_size --hidden_size $lstm_hidden_size --num_layers ${lstm_layers} --tag_emb_size $slot_tag_embedding_size \
      --st_weight ${balance_weight} ${word_lowercase:+--word_lowercase} ${read_word2vec_inText:+--read_input_word2vec ${read_word2vec_inText}} \
      ${fix_word2vec_inText:+--fix_input_word2vec} ${word_digit_features:+--word_digit_features}
