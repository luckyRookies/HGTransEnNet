...
from hyedge import degree_hyedge, degree_node, count_hyedge, count_node


class HyConv(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, bias=True) -> None:
        super().__init__()
        self.theta = Parameter(torch.FloatTensor(in_ch, out_ch))
    ...
    def gen_hyedge_ft(self, x: torch.Tensor, H: torch.Tensor, hyedge_weight=None):
        ...
        hyedge_norm = hyedge_norm.to(x.device)
        # print('device:', x.device, hyedge_norm.device)

        x = x[node_idx] * hyedge_norm.unsqueeze(1)
        x = torch.zeros(hyedge_num, ft_dim).to(x.device).scatter_add(0, hyedge_idx.unsqueeze(1).repeat(1, ft_dim).to(x.device), x)
        return x
    ...
    def gen_node_ft(self, x: torch.Tensor, H: torch.Tensor):
        ...
        x = x[hyedge_idx] * node_norm.unsqueeze(1).to(x.device)
        x = torch.zeros(node_num, ft_dim).to(x.device).scatter_add(0, node_idx.unsqueeze(1).repeat(1, ft_dim).to(x.device), x)
        return x
